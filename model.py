...
def inference_graph(word_vocab_size,
    ...
    input_ = tf.placeholder(tf.int32, shape=[batch_size, num_unroll_steps], name="input")

    ''' First, embed characters '''
    with tf.variable_scope('Embedding'):
        word_embedding = tf.get_variable('word_embedding', [word_vocab_size, word_embed_size])
        ...
        clear_char_embedding_padding = tf.scatter_update(word_embedding, [0], tf.constant(0.0, shape=[1, word_embed_size]))

        # [batch_size, num_unroll_steps, word_embed_size]
        input_embedded = tf.nn.embedding_lookup(word_embedding, input_)

    input_cnn = input_embedded
    if dropout > 0.0:
        input_cnn = tf.nn.dropout(input_cnn, keep_prob = 1. - dropout)
    ...
    with tf.variable_scope('LSTM'):
        ...
        input_cnn2 = [tf.squeeze(x, [1]) for x in tf.split(input_cnn, num_unroll_steps, 1)] # len * <batch, wemb>
